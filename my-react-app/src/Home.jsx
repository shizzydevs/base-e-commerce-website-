...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-stone-100 p-4 md:p-8 w-full max-w-7xl mx-auto">
      ...
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-stone-800 mb-6">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-white">Lebanon Store Hub</h1>
      ...
      {loading ? (
        <div className="py-20 text-center text-stone-500 text-sm font-semibold">Loading items for {currentCity?.name}...</div>
      ) : products.length === 0 ? (
        <div className="py-20 text-center bg-stone-900/30 border border-stone-800 rounded-3xl p-8">
          <p className="text-stone-400 text-sm font-semibold mb-2">No items found for {currentCity?.name} in this category.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {products.map((product) => (
            <div
              key={product.id}
              className="bg-stone-900 border border-stone-800 rounded-2xl p-4 flex flex-col justify-between hover:border-stone-700 transition-all shadow-xl group"
            >
              <div>
                {/* Store Header Info */}
                ...
                {/* Product Image */}
                {product.image_url ? (
                  <div className="h-44 w-full bg-stone-950 rounded-xl mb-3 overflow-hidden">
              ...
              <div>
                ...
                <div className="pt-3 border-t border-stone-800 flex items-center justify-between">
                  <div>
                    <span className="text-lg font-black text-emerald-400">${product.price_usd}</span>
                    {product.price_lbp && (
